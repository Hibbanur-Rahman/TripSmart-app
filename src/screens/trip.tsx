...
const Trip = () => {
  ...
  const trips = {
    upcoming: [
      { id: 1, destination: 'Paris, France', date: 'May 15-20, 2025', image: parisImage, weather: 'Sunny, 22°C' },
      { id: 2, destination: 'Tokyo, Japan', date: 'June 8-16, 2025', image: tokyoImage, weather: 'Cloudy, 19°C' },
    ],
    past: [
      { id: 3, destination: 'New York, USA', date: 'March 3-8, 2025', image: parisImage, weather: 'Rainy, 15°C' },
      { id: 4, destination: 'Bali, Indonesia', date: 'January 12-20, 2025', image: tokyoImage, weather: 'Sunny, 30°C' },
    ]
  };

  const TripCard = ({ trip }) => (
    <View style={tw`bg-black/70 rounded-xl shadow-md mb-4 overflow-hidden`}>
      <View style={tw`h-40 `}>
        {/* Using placeholder for image - in real app, use actual image */}
        <View style={tw`absolute  opacity-20 h-full w-full`}>
            <Image source={trip?.image} style={[tw``]}/>
        </View>
        <View style={tw`w-full p-4 absolute bottom-0 left-0 right-0 z-[10]`}>
          <Text style={tw`text-white font-bold text-xl`}>{trip.destination}</Text>
  ...
  const AddTripForm = () => (
    <View style={tw`bg-white rounded-xl shadow-md p-4 mb-4`}>
      <Text style={tw`text-lg font-bold mb-4 text-gray-800`}>Create New Trip</Text>
      ...
      <View style={tw`flex-row mb-4`}>
        <View style={tw`flex-1 mr-2`}>
          ...
          <TouchableOpacity style={tw`border border-gray-300 rounded-lg p-2 flex-row items-center`}>
      ...
      <ScrollView style={tw`flex-1 p-4`}>
        ...
        <View style={tw`flex-row mb-4 bg-gray-200 rounded-lg p-1`}>
          <TouchableOpacity 
            ...
            style={tw`flex-1 py-2 rounded-md ${activeTab === 'upcoming' ? 'bg-white shadow' : ''}`}
          >
            <Text style={tw`text-center font-medium ${activeTab === 'upcoming' ? 'text-blue-500' : 'text-gray-600'}`}>Upcoming</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            onPress={() => setActiveTab('past')}
            style={tw`flex-1 py-2 rounded-md ${activeTab === 'past' ? 'bg-white shadow' : ''}`}
          >
            <Text style={tw`text-center font-medium ${activeTab === 'past' ? 'text-blue-500' : 'text-gray-600'}`}>Past</Text>
          </TouchableOpacity>
        </View>
        ...
        {trips[activeTab].length > 0 ? (
          ...
          <View style={tw`bg-white rounded-xl shadow-sm p-8 items-center justify-center`}>
            <Plane size={48} color="#9CA3AF" style={tw`mb-4`} />
            <Text style={tw`text-gray-500 text-center text-lg`}>
              {activeTab === 'upcoming' ? "No upcoming trips planned yet!" : "No past trips to show."}
            </Text>
            {activeTab === 'upcoming' && (
              <TouchableOpacity 
